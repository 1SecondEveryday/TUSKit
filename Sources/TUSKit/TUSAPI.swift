...
final class TUSAPI {
    ...
    func upload(data: Data, range: Range<Int>?, location: URL, metaData: UploadMetadata, customHeaders: [String: String], completion: @escaping (Result<Int, TUSAPIError>) -> Void) -> URLSessionUploadTask {
        ...
        observeProgressForCompatibilityIfNeeded(task: task, totalBytesExpectedToSend: Int64(data.count))
    ...
    func upload(fromFile file: URL, offset: Int = 0, location: URL, metaData: UploadMetadata, customHeaders: [String: String], completion: @escaping (Result<Int, TUSAPIError>) -> Void) -> URLSessionUploadTask {
        ...
        observeProgressForCompatibilityIfNeeded(task: task, totalBytesExpectedToSend: Int64(length))
    ...
    private func observeProgressForCompatibilityIfNeeded(task: URLSessionTask, totalBytesExpectedToSend: Int64) {
        let canUseTaskDelegate: Bool = if #available(iOS 15.0, macOS 12, *) {
            !session.configuration.sessionSendsLaunchEvents
        } else {
            false
        }

        guard !(session.delegate is SessionDataDelegate), !canUseTaskDelegate else { return }
        observeProgressForCompatibility(task: task, totalBytesExpectedToSend: totalBytesExpectedToSend)
    }
